...
export const CardWrapper = ({ title, link, backdrop, children }: Props) => {
  return (
    <Link
      prefetch={false}
      title={title}
    ...
    >
      {backdrop && (
        <div
          className={`absolute top-0 bottom-0 left-o right-0 bg-background transition-opacity ease-in-out group-hover:opacity-60'
          }  z-10 w-full h-full`}
        ></div>
      )}
      {children}
